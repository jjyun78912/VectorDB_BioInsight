...
| Pan-Cancer (17종) | 11,000+ | 0.988 | ✅ |

**Pan-Cancer 17종 검증 결과** (2026-01-16):
- 전체 정확도: 98.8% (84/85 samples)
- SKCM 오분류 1건: SKCM→HNSC (점막 흑색종 유사 발현 패턴)

**혼동 가능 암종 쌍 탐지** (`CONFUSABLE_CANCER_PAIRS`):
...
**2차 검증 기능** (`secondary_validation`):
- `confidence_gap`: Top1 - Top2 확률 차이 (< 15%면 불확실)
- `is_confusable_pair`: 생물학적 혼동 가능 쌍 여부
- 조직 특이 마커 발현 검사로 보정 가능

### Hub Gene vs Driver Gene 개념
